...
disp('%% Ground Truth is not =  HP1 to validate the result %%');
disp(' Which Means, it is not satisfying the equation P2(1,:) = H*P1(1,:)');
GroundTruth = (P2(1,:)/P2(1,3))'   % Ground Truth  = P2(1,:)
disp('The small error is due to the Noise Added to the points of second Camera');
disp(' ');
...
H4pt = H4pt/H4pt(3,3)
disp('%%% You can see, the H and H4pt is not exactly the same %%%');
disp('The small error is due to the Noise Added to the points of second Camera');
disp(' ');
...
disp('%% Ground Truth is not = H_Valid to validate the result %%');
disp('The small error is due to the Noise Added to the points of second Camera');
disp(' ');
...
disp('But The small error is due to the Noise Added to the points of second Camera');
Estimated_H = H/H(3,3)
...
% Using RANSAC, We can reduce the error and can come closer to the Original
% H but we can't just nullify the effect of noise on the output.
